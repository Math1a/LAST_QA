...
lapses = 1 ; % number of lapses = iterations of the main loop 
...
% connect slaves 
P.connect
% The pause is irrelevant, P.connect will block the code, so there is no need in pausing 
% pause(60)

% allow logging for SDK errors (in slaves) and save to stderror log files 
for i=1:4
    P.Camera{i}.classCommand('DebugOutput = true');
    P.Camera{i}.classCommand('DebugLogLevel = 7'); % define verbose level for SDK errors
...
%% main 
lapse = 1; % could have just used a for loop for the lapse
while lapse<=lapses
    Count(lapse) = lapse;
    ...
    % make an average of the colling power and temperature
...
%% gradually warms cams to ~room temp before switching off
for i=1:4
